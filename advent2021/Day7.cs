...
namespace advent2021
{
    internal class Day7
    {
        ...
        public void StartUp()
        {
            input = GetInput().Replace(","," ").Split().Where(x => x.Trim() != " ").Select(int.Parse).ToList();
        ...
        public int Part1()
        {
            for (int i = 0; i < inputMax; i++)
            {
                ...
                for(int j = 0; j < inputCount; j++)
                {
                    ...
                    //Part2: extra fuel cost
                    for (int k = 0; k < holder; k++)
                    {
                        extraFuel += k;
                    }
                    fuelCheck = fuelCheck + holder + extraFuel;
                }

                if (leastFuel == 0)
                    leastFuel = fuelCheck;
                if (fuelCheck < leastFuel)
                    leastFuel = fuelCheck;
            }
            return leastFuel;
        }
